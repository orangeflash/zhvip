...
Page({
  ...
  bindinput: function (e) {
    var inputvalue = Number(parseFloat(e.detail.value).toFixed(2))
    if (inputvalue.toString() == 'NaN') {
      inputvalue = 0
    }
    console.log(e.detail.value, e.detail.value.split('.').length,inputvalue,this.data.yhqfull,this.data.yhqname,this.data.yhqkdje)
    if (Number(inputvalue) < Number(this.data.yhqfull)){
      wx.showModal({
        title: '温馨提示',
    ...
    if (this.data.yhqname == '折扣券') {
      ...
      this.setData({
        kdje: ((1 - Number(that.data.yhqkdje) * 0.1) * Number(inputvalue)).toFixed(2),
      })
    }
    else {
      console.log(this.data.yhqname)
    }
    console.log(inputvalue,this.data)
    var discount =1-Number(this.data.discount)/100;
    console.log(discount)
    var discounttext = (Number(inputvalue) * discount).toFixed(2);
    this.setData({
      discounttext: discounttext,
      xfje: Number(inputvalue),
    })
    if (e.detail.value != '' && e.detail.value.split('.').length<=2) {
      this.setData({
        disabled: false,
      })
    }
  ...
  onLoad: function (options) {
    ...
    app.util.request({
      ...
      success: function (res) {
        ...
        var length = res.data.announcement.length * that.data.size;
